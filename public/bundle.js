...
var css = {"container":"styles-mod_container__Klf1J"};
...
var css$1 = {"grid":"grid-mod_grid__3yhIS"};

var css$2 = {"node":"cell-mod_node__1-__j"};

const Cell = ({ x, y, rem, isBlocked, isActive, isInput, isOutput }) => (h("rect", { height: rem, width: rem, x: x * rem, y: y * rem, "data-blocked": isBlocked, "data-active": isActive, "data-input": isInput, "data-output": isOutput, className: css$2.node }));

const rem = 20;
const GridCells = ({ source, activeCellId, inputCellId, outputCellId }) => {
    return (h("svg", { width: source[0].length * rem, height: source.length * rem, className: css$1.grid }, // prettier-ignore
    source.map((height, y) => height.map((value, x) => {
        const id = chordsToId(x, y);
        return (h(Cell, Object.assign({ key: id }, { x, y, rem }, { isBlocked: Boolean(value), isActive: id === activeCellId, isInput: id === inputCellId, isOutput: id === outputCellId })));
    }))));
};
var Grid$1 = C$1(GridCells);
const chordsToId = (x, y) => `${x},${y}`;

var css$3 = {"action":"styles-mod_action__2Qupr"};

const Actions = ({ onCreate, onRun }) => (h(d, null,
    h("button", { className: css$3.action, onClick: onCreate, children: "generate" }),
    h("button", { className: css$3.action, onClick: onRun, children: "run" })));

var css$4 = {"container":"styles-mod_container__2G1Xn","dl":"styles-mod_dl__2Q3C4","error":"styles-mod_error__34N1L"};

const Legend = ({ activeCellId, currentStep, success }) => (h("legend", { className: css$4.container },
    !success && h("div", { className: css$4.error }, "no way out"),
    h(Node, { dt: "active cell", dd: activeCellId }),
    h(Node, { dt: "total steps", dd: currentStep })));
var Legend$1 = C$1(Legend);
const Node = ({ dt, dd }) => (h("dl", { className: css$4.dl },
    h("dt", { children: dt }),
    h("dd", { children: dd })));
...
const source = new Grid(25 /* width */, 25 /* height */, 0.3 /* proximity */, 0 /* input index */, 25 /* output index */);
// set up the graph on the grid basis
const graph = new Graph(source.instance);
// calculate the shortest path
const dfs = new Dfs(graph.instance, '0,0' /* top left */, '24,25' /* bottom right */);
const Root = () => {
    ...
    return (h("section", { className: css.container },
        h(Grid$1, { source: source.instance, inputCellId: dfs.input, outputCellId: dfs.output, activeCellId: activeId }),
        h("div", null,
            h(Actions, { onRun: run, onCreate: create }),
            h(Legend$1, { success: dfs.success, currentStep: currentStep, activeCellId: activeId }))));
};
var Root$1 = C$1(Root);
