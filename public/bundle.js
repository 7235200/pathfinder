...
class Grid {
    constructor(width, height, proximity) {
        this.createCellValue = () => {
            return Math.random() < this.proximity ? 1 : 0;
        };
        this.createRow = (size = this.width) => {
            return new Array(size).fill(1).map(this.createCellValue);
        };
        this.createGrid = (width = this.width, sourceHeight = this.height) => {
            const grid = [];
...
const App = () => {
    ...
    graph.print();
    return h("div", null, "lol");
};

H(h(App, null), document.getElementById('root'));
